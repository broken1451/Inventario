...
export class InterceptorService implements HttpInterceptor {
  ...
  intercept(req: HttpRequest<any>, next: HttpHandler): any {
    ...
    if (request.url.includes(API.login)) {
      ...
      return next.handle(request).pipe(catchError(this.manejarErr));
    } else if (request.url.includes(API.pcCreate) && token) {
      request = req.clone({
        setHeaders: {
          'x-token': `${token}`,
        },
      ...
      return next.handle(request).pipe(catchError(this.manejarErr));
    } else if (request.url.includes(API.pcUpdate) && token) {
      request = req.clone({
        setHeaders: {
          'x-token': `${token}`,
        },
      ...
      return next.handle(request).pipe(catchError(this.manejarErr));
    } else if (request.url.includes(API.memoryDeleted) && token) {
      request = req.clone({
        setHeaders: {
          'x-token': `${token}`,
        },
      ...
      return next.handle(request).pipe(catchError(this.manejarErr));
    } else if (request.url.includes(API.memoryCreate) && token) {
      request = req.clone({
        setHeaders: {
          'x-token': `${token}`,
        },
      ...
      return next.handle(request).pipe(catchError(this.manejarErr));
    } else if (request.url.includes(API.memoryUpload) && token) {
      request = req.clone({
        setHeaders: {
          'x-token': `${token}`,
        },
      ...
      return next.handle(request).pipe(catchError(this.manejarErr));
    } else if (request.url.includes(API.otrosUpdate) && token) {
      request = req.clone({
        setHeaders: {
          'x-token': `${token}`,
        },
      ...
      return next.handle(request).pipe(catchError(this.manejarErr));
    } else if (request.url.includes(API.otrosDeleted) && token) {
      request = req.clone({
        setHeaders: {
          'x-token': `${token}`,
        },
      ...
      return next.handle(request).pipe(catchError(this.manejarErr));
    } else if (request.url.includes(API.otrosCreate) && token) {
      request = req.clone({
        setHeaders: {
          'x-token': `${token}`,
        },
      ...
      return next.handle(request).pipe(catchError(this.manejarErr));
    } else if (request.url.includes(API.otrosUpload) && token) {
      request = req.clone({
        setHeaders: {
          'x-token': `${token}`,
        },
      ...
      return next.handle(request).pipe(catchError(this.manejarErr));
    } else if (request.url.includes(API.otros) && token) {
      request = req.clone({
        setHeaders: {
          'x-token': `${token}`,
        },
